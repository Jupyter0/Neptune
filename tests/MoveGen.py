...
class FenTests:
    test_fens = [
        ...
        "K5rk/1N1P4/8/8/8/8/1P6/8 w - - 0 1",
    ]
    ...
    perftResults = [
        ...
        [18 , 270 , 4699 , 79355 , 1533145 , 28859283],
        [24 , 496 , 9483 , 182838 , 3605103 , 71179139],
        [7, 19, 213, 1103, 16047, 80164],
        [6, 61, 597, 7735, 101481, 1450581]
    ]
...
if __name__ == "__main__":
    ...
    q = queue.Queue()
    t = threading.Thread(target=reader_thread, args=(engine.stdout, q))
    t.daemon = True
    t.start()
    ...
    # --- Main loop ---
    for testFenI in range(len(FenTests.test_fens)):
        fen = FenTests.test_fens[testFenI]
        engine.stdin.write(f"position fen {fen}\n")
        engine.stdin.flush()

        for depth in range(len(FenTests.perftResults[testFenI])):
            engine.stdin.write(f"go perft {depth + 1}\n")
            engine.stdin.flush()
            ...
            if nodesFound == FenTests.perftResults[testFenI][depth]:
                ...
                print(f"\r{fen} at depth {depth + 1}: FAIL (got {nodesFound}, expected {FenTests.perftResults[testFenI][depth]})")
                exit(1)
